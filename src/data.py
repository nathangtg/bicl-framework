...
def generate_diverse_tasks(config: dict) -> List[Tuple[Dataset, Dataset]]:
    ...
    Returns:
        A list of tasks, where each task is a tuple of (train_dataset, test_dataset).
    """
...
class TaskSplitter(Dataset):
    ...
    This is essential for creating 'domain-incremental' task sequences like Split CIFAR.
    """
...
class PermutedMNIST(Dataset):
    """
    A custom Dataset for the Permuted MNIST benchmark. Each task uses the full
    MNIST dataset but with a different fixed permutation of the input pixels.
    """
...
def get_benchmark_tasks(config: dict) -> List[Tuple[Dataset, Dataset]]:
    ...
    This function now supports Split CIFAR-10, Split CIFAR-100, and Permuted MNIST.
    """
    data_config = config['data']
    ...
    if benchmark_name.lower() in ['cifar10', 'cifar100']:
        ...
        train_set = dataset_loader(root=data_path, train=True, download=True, transform=transform)
    ...
    # --- Permuted MNIST Logic ---
        ...
        for _ in range(num_tasks):
            # For each task, create a new random permutation of pixels
